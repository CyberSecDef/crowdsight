...
Built as one view with four phases derived from what exists rather than from a wizard step, so opening `/graphs/<id>` for a finished graph lands on the graph and opening it for a parked one lands on the review. Cytoscape.js, one dependency with no transitive tree, lazy-loaded with the route so the 466 KB chunk is only fetched by someone who opens a graph. Verified against the live sealed stack: **36 HTTP checks, 89 unit tests, 24 browser tests**, including the whole stage driven end to end against the local model — upload, ontology proposal, an edit, approval, extraction, and a drawn graph — in 20 seconds.

**The identifier rule is now implemented twice, so it is pinned by a shared fixture.** The editor shows what a typed name will become before it is submitted, because a name silently changing between the form and the graph is a nasty thing to find later. That means `to_identifier` exists in Python for storage and in JavaScript for the preview, and two implementations of one rule drift. `backend/tests/fixtures/identifier_cases.json` is the contract and both suites assert against it — 30 cases, run from Python and from Vitest. Writing it immediately caught an error in my own expectations rather than in the code: `3rd sector` yields nothing at all, because a label cannot begin with a digit, and the correct behaviour is to reject rather than mangle.

**Deleting an entity type silently deletes relationships.** `Ontology._dedupe_and_link` drops any relationship whose endpoints are not in the ontology, without a warning, because such a relationship cannot be extracted. That is right for a generated ontology and surprising for a hand-edited one, so the editor computes what a removal would take with it and asks first. Renaming is handled rather than warned about: relationship endpoints follow the rename, which is what an operator means by renaming a type.

**Client-side validation is a courtesy, and the test that matters proves no request was made.** A unit test can only show the predicate returned false; the browser test asserts that a `.exe` selected in the file picker produced no POST to `/api/graph/upload` at all. The mirrored limits are checked against the running config by `scripts/verify_frontend.sh`, because a mirror that has drifted is worse than no mirror — the UI accepts the file, the upload runs, and the refusal arrives late looking like a bug.

**A crash that read as a routing problem.** The ontology editor copies its prop into a local draft, and it used `structuredClone` to do it. `structuredClone` throws `DataCloneError` on a Vue reactive Proxy, which is exactly what a prop is. The failure was in `setup`, so the component never rendered and the parent's subtree blanked with it: the page kept its heading and lost everything else, with no error visible on screen. It cost a five-minute live test run to see, and the fix is a JSON round-trip — the ontology is pure JSON, so that is both sufficient and honest about the data.

**A second, latent bug found while chasing the first.** The editor was bound with `v-model`: it deep-watched its draft and emitted, the parent wrote the value back, the parent's watcher re-cloned it into the draft, which emitted again. An infinite update loop that never actually ran, because the clone threw first. Nothing upstream needs to see every keystroke, so the binding is now one-way and the draft leaves only on approve.
